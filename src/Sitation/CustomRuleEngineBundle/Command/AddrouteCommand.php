...
namespace Sitation\CustomRuleEngineBundle\Command;
...
class AddrouteCommand extends ContainerAwareCommand
{
   ...
   /* protected function __construct(ContainerInterface $container)
    ...
    {
        $this->setName('sitation:addroutecustomrulesengine');
    }
       ...
       // public ContainerBuilder $container
        try{
            ...
            $yaml = Yaml::parse(file_get_contents($this->container->get('kernel')->getRootDir() .'/config/routing.yml'));
                ...
                $wissend_custom_array['resource']='@SitationCustomRuleEngineBundle/Resources/config/routing.yml';
                $wissend_custom_array['prefix']='/sitation';
                if(!array_key_exists('sitation_custom_rule_engine', $yaml)){
                    $yaml['sitation_custom_rule_engine'] =$wissend_custom_array;
